...
1. **[仮想ネットワーク ゲートウェイ]** ページで **[更新]** を選び、仮想ネットワーク ゲートウェイの一覧に、名前が **WAC-Created-vpngw-ID_NO** で始まる新しいエントリが表示されることを確認します。
...
1. ローカル Administrator アカウントのパスワードを入力するように求めるダイアログが表示されたら、「**Pa55w.rd1234**」と入力します
   ...
   >**注**: WSMan のインストールに関するエラーが発生した場合は、以下の手順に従ってセットアップを完了し、ラボに進みます。
   - Azure portal で、**仮想マシン**を検索し、az800l04-vmwac VM を選択します。
   - 左側で **[接続]** をクリックし、**[RDP ファイルのダウンロード]** を選択します。
   - ファイルを開き、手順 9 と 10 で言及した資格情報を使用して接続します。
   - ログインしたら、開いているすべてのウィンドウを閉じ、Edge ブラウザーを開いて、**Windows Admin Center** を検索します。
   - 最初の [戻る] リンクをクリックし、ページで **[Windows Admin Center のダウンロード]** を選択します。
   - ファイルを開き、 **[これらの条件に同意します]** チェック ボックスをオンにして、**[次へ]** を 5 回クリックします。
   - **[インストール]** をクリックし、**[完了]** をクリックします。
...
>**注**: 受信ポートの規則が表示されない場合は、**[仮想マシン]** タブに移動し、az800l04-vmwac を開き、**[DNS 名]** をコピーして、新しいブラウザー タブに https:// 形式で貼り付けます。 出力を調べます。
