...
1. **SEA-ADM1** で Microsoft Edge を起動して `https://portal.azure.com` で Azure portal に移動し、このラボで使うサブスクリプションの所有者ロールを持つユーザー アカウントの資格情報を使ってサインインします。
...
1. 他のすべての設定については既定値をそのまま使用し、 **[確認]** 、 **[作成]** の順に選択します。
1. ストレージ アカウントが作成された後、**[デプロイ]** ページで、**[リソースに移動]** を選択します。
1. **[ストレージ アカウント]** ページで、**[ストレージ ブラウザー]** を選択し、**[ファイル共有]** メニューを選択して、**[+ ファイル共有]** を選択します。
...
1. **[ファイル共有]** メニューに戻り、**share1** の省略記号を選択して、表示された一覧で **[スナップショットの表示]** を選択します。
1. **[ファイル共有スナップショット]** ページで、**[スナップショットの追加]** を選択し、**[OK]** をクリックします。
1. **[ファイル共有]** メニューに戻り、**share1** を表しているボックスを選択して、**[接続]**、**[スクリプトを表示]** の順に選択し、**[クリップボードにコピー]** ボタンを使用してスクリプトをコピーし、**[接続]** タブを閉じます。
