...
# ラボ: ハイブリッド シナリオでの Windows Admin Center の使用
...
このラボでは、使用可能な VM 環境と Azure サブスクリプションを使用します。 ラボを開始する前に、Azure サブスクリプションと、そのサブスクリプションの所有者ロールまたは共同作成者ロール、およびそのサブスクリプションに関連付けられている Microsoft Entra テナントのグローバル管理者ロールを持っているユーザー アカウントがあることを確認します。
...
1. メッセージが表示されたら、講師から提供された資格情報を使用して認証します。
