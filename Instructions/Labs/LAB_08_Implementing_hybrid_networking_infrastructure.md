...
1. **[チェック]** を選び、接続チェックの結果が返されるまで待ちます。 状態が "**到達可能**" であることを確認します。 ネットワーク パスを確認します。接続が直接であり、仮想マシン間に中間ホップがないことに注意してください。
    ...
    > 2. **RunPowerShellScript** を選択します。
    > 3. 以下のコマンドを実行して、到達可能であることを確認します
    >    **Test-NetConnection -ComputerName 10.82.0.4 -Port 3389**
...
1. 接続チェックの結果が返されるまで待ちます。 状態が "**到達不能**" であることに注意してください。
    ...
    >**代替手段**
    > 1. 仮想マシン **az800l08-vm1** の画面に移動し、**Run Command Script** を選択します。
    > 2. **RunPowerShellScript** を選択します。
    > 3. 以下のコマンドを実行して、**到達不能** であることを確認します
    >    **Test-NetConnection -ComputerName 10.82.0.4 -Port 3389**
...
1. **az800l08-vm0** 仮想マシン ページで、**Networking** を選択し、**az800l08-nic0** をクリックします。

1. **az800l08-nic0** ネットワーク インターフェイス ページで、 **[IP Configuration/IP 構成]** ページを表示し、 **[IP Forwarding/IP 転送]** を **Enabled/有効** にします。
...
1. Azure portal で **az800l08-vm0** 仮想マシン ページに戻り、 **[Run Command/実行コマンド]** 操作の一覧から **RunPowerShellScript** を使って、リモート アクセス Windows Server ロールをインストールする次のコマンドを実行します。
...
1. Azure portal で **[Route Tables/ルート テーブル]** ページに移動し、次の設定を使用してルート テーブルを作成します (他の設定は既定値のままにします)。
...
1. 新しく作成されたルート テーブル **az800l08-rt12** のページに移動し、**Route** をクリックして、次の設定でルートを追加します。

    | 設定 | 値 |
    | --- | --- |
    | ルート名 | **az800l08-route-vnet1-to-vnet2** |
    | アドレス プレフィックス | **IP Address** |
    | 宛先IPアドレス | **10.82.0.0/20** |
    | ネクストホップの種類 | **仮想アプライアンス** |
    | 次ホップ アドレス | **10.80.0.4** |

    > **注**: **10.80.0.4** は、**az800l08-vm0** のプライベート IP アドレスを表します。 

1. **az800l08-rt12** ルート テーブル ページで、**Subnets** を選択し、ルート テーブル **az800l08-rt12** を次のサブネットに関連付けます。
...
1. いま作成した **contoso.org** DNS プライベート ゾーン ページに移動します。

1. **contoso.org** プライベート DNS ゾーン ページで、**Virtual network links** を選択し、**az800l08-vnet0** にプライベートDNSゾーンをリンクします (その他は既定値のままにします)。
    ...
    | Enable auto registration/自動登録を有効にする | オン |

    >**注:** 仮想ネットワーク リンクが作成されるまで待ちます。 これにかかる時間は 1 分未満です。

1. 前の手順を繰り返し、仮想ネットワーク **az800l08-vnet1** と **az800l08-vnet2** に対して、それぞれ **az800l08-vnet1-link** と **az800l08-vnet2-link** という仮想ネットワーク リンクを作成します。自動登録を有効にすることを忘れないでください。

1. **contoso.org** プライベート DNS ゾーン ページで、**contoso.org** プライベート DNS ゾーンの **[Overview/概要]** ペインに移動します。DNS レコード セットの一覧を確認し、**A** レコードの **az800l08-vm0**、**az800l08-vm1**、**az800l08-vm2** が "**自動登録**" として一覧に表示されていることを確認します。
...
1. 次の設定で DNS ゾーンを作成します (その他は既定値のままにします)。
    ...
    | 名前 | ドメイン名を”適当に”決定してください（contoso.com 等）。存在するドメイン名でも問題ありません。 |
...
1. DNS ゾーン ページで、**Name server 1** の FQDN をメモします。
...
1. **SEA-ADM1** で、**Windows PowerShell** を起動します。
