...
1. **[次へ: 管理 >]** を選択し、すべての設定を既定値のままにします。
   
1. **[次へ: 監視 >]** を選択し、 **[仮想マシンの作成]** ページの **[管理]** タブで、次の設定を指定します。他のすべての設定は既定値のままにします。
...
1. **[Settings \| Defender plans]** ページで、**[Enable all Microsoft Defender for Cloud plans]** タイルが選ばれていることを確認します。 メッセージが表示されたら、**[Defender for API]** プランをスキップします。 上部の **[Settings & monitoring]** を選びます。
1. **[Settings & monitoring]** ページで、使用できる他のオプションを確認します。
...
1. **[Just In Time VM アクセス]** ページで、**[構成済み]**、**[未構成]**、および **[サポート対象外]** のタブを確認します。
   ...
   >**注**:新しくデプロイされた VM が **[Unsupported]** タブに表示されるまで、最大で 24 時間かかる場合があります。この時点でこの VM がサポートされないのは、Just-In-Time VM アクセスにはネットワーク セキュリティ グループが必要であるためです。 待つ代わりに、次の演習を続けてください。
...
1. Azure portal で **AZ800-L0601-RG** のページに戻り、リソースの一覧で、新しく作成されたネットワーク セキュリティ グループ **az800L06-vm0-nsg1** を表すエントリを選択します。
